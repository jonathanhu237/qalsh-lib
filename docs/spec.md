...
The current follow-up effort is [QALSH library reuse, selectable persistent indexes, and performance recovery](../.scratch/qalsh-library-compatibility/spec.md), published with status `ready-for-agent`. It supersedes conflicting storage, extension, and acceptance requirements below. This document retains the original migration scope and historical context; its proposed 5% allowance is not an approved gate.
...
Build a library with an in-memory sorted-array index and two persistent layouts: B+ trees by default and sorted-array files selected through a construction option. The library owns traversal mechanics, point access invocation, distance computation, evaluation deduplication, top-k maintenance, and the search execution loop. The updated storage scope is recorded in [ADR 0001](adr/0001-selectable-persistent-index-layout.md); this specification does not itself authorize implementation.
...
- Support a static in-memory sorted-array index and persistent B+ tree and sorted-array layouts. Persistent construction defaults to B+ trees; an option selects sorted-array files. mmap is an I/O mechanism that may serve either persistent layout, not another index structure.
- The in-memory index is built and queried within the process and does not require save/load support. Both persistent layouts support construction followed by an independent open/query lifecycle. Persist the backend identity and select the matching implementation when opening an existing index.
...
- The library owns both persistent formats. Reading either consumer's historical index format is not required. Migrated construction commands must produce a library format, and migrated queries must open it.
...
- qalsh4c must use library indexing and search execution for its current QALSH memory and disk workflows. Use the library memory index for its memory mode; migrated disk construction defaults to B+ trees and supports the sorted-array file option. Do not silently drop the disk scenario because its previous backend was mmap over flat arrays.

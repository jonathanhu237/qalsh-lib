...
Status: **targeted fix attempt 1 complete; prior comparison evidence superseded and acceptance remains open**.
...
- Baseline executables were built separately from `/home/jonathanhu237/code/qalsh4c-baseline-fixed` and `/home/jonathanhu237/code/qalsh-h-baseline-94be153`; migrated executables were built from the Centaurus rsync copies.
...
C estimates and relative errors matched the pinned baseline at printed precision for every listed norm/direction/mode. H fixed-seed results were default k=1 `1.113364` / `1.00%`, default k=100 `1.070133` / `7.02%`, complete k=1 `1.011630` / `47.00%`, and complete k=100 `1.021711` / `30.62%`; migrated runs reported zero partial queries. H's default k=100 ratio is `1.070133` versus pinned baseline `1.069267`, a small approved immediate-evaluation/order difference that remains reported rather than hidden. The post-atomic Docker query reproduced the same ratios/recalls with zero partial queries.

The available C/H timing evidence is not a performance pass: the proposed 5% margin is exceeded in most L1/disk/H scenarios, and no numeric slowdown allowance has been approved. C L2 memory is within the proposed margin in that pre-final-fix run. MNIST and larger H real-data rows remain unavailable in this checkout. Post-fix fixed-seed H control output is archived under `final-atomic/docker/h-query-final-round.out`; it confirms quality/partial-query behavior but is not a replacement timing matrix.

## Final acceptance-review finding closure

Review 3 initially reported three changes-required findings: rounded bounded-L2 tie pruning, the pre-manifest H artifact-directory durability window, and unbounded versioned-generation retention. The parent fixed each directly; `review-1/review-3-findings.md` records the findings and closure. The bounded-L2 regression and H generation/failed-rebuild controls were rerun on Centaurus and in the Linux GCC 15 Docker environment.
...
- The targeted reviews approved the quantum-one/max-budget scans, B+ leaf-chain validation, portable mapped-entry decoding, scalar strategy fast-path integration, complete C index metadata validation, and manifest-only H publication design; see `review-1/review-2-findings.md` and `review-1/review-3-findings.md`. The final fixes add tie-preserving bounded-L2 pruning, pre-manifest H directory durability, and post-commit H generation cleanup. The remaining open decision is performance/scope plus final immutable library revision pinning.
